...
def inference(input,train,regularizer):
    ...
    with tf.variable_scope("cov1"):
        conv1_weights = tf.get_variable("conv1_weights",shape=[COV1_SIZE,COV1_SIZE,INPUT_DEEPTH,COV1_DEEPTH],initializer=tf.truncated_normal_initializer(stddev=1.0))
        ...
        conv1_result = tf.nn.relu(tf.nn.bias_add(conv1,conv1_biases))
    ...
    with tf.variable_scope("conv2"):
        conv2_weights = tf.get_variable("conv2_weights",shape=[COV2_SIZE,COV2_SIZE,COV1_DEEPTH,COV2_DEEPTH],initializer=tf.truncated_normal_initializer(stddev=1.0))
        ...
        conv2_result = tf.nn.relu(tf.nn.bias_add(conv2,conv2_biases))
    ...
    with tf.variable_scope("fully_connetc2"):
        fc2_weights = tf.get_variable("fc2_weights",shape=[FULLY_C1,INPUT_LABELS],initializer=tf.truncated_normal_initializer(stddev=1.0))
